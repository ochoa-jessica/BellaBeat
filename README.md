...
<p>According to the [SleepFoundation](https://www.sleepfoundation.org), we burn around 50 calories an hour while we sleep - and we are able to see that in the graph above. Observe that as the users wake up and start moving, the number of burned calories increases, peaking around the same time that the users are most active during the day.</p>
